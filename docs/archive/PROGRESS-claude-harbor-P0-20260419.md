...
- [x] **P0.4** hook/statusline/installer packages at `current/claude-harbor/{hook,statusline,installer}/`. Reviews: code ✅ sec ✅ func ✅. Final: hook 20, statusline 18, installer 25 tests; server 29, proxy 30, wrapper 21 unchanged; tsc clean everywhere. Hardening: HARBOR_URL http/https allowlist + userinfo stripping, stdin inactivity timeout (hook 1500ms / statusline 300ms), control-char strip on statusline output (512-char cap), atomic write via O_EXCL + crypto.randomBytes, backup path with random suffix, 1 MiB settings.json size cap, strict sidecar schema validation, structural hook-entry equality (not JSON.stringify).

## Next Steps
- **P0 complete.** Archiving PROGRESS. Next phase P1 (full hooks + session correlation) starts with a fresh PROGRESS.md when the user kicks it off.
